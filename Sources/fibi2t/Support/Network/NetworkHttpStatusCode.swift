...
@objc public enum NetworkHttpStatusCode: Int {
...
public extension NetworkHttpStatusCode {
  ...
  private func isIn(range: ClosedRange<NetworkHttpStatusCode.RawValue>) -> Bool {
    return range.contains(rawValue)
  }
}

fileprivate final class BundleClingClass {

  @available(*, unavailable) private init () {}
}

extension NetworkHttpStatusCode: CustomStringConvertible {

  /// - returns: a localized description suitable for debug
  public var description: String {
    return NSLocalizedString( "http_status_\(rawValue)",
      tableName: "HttpStatusEnum",
      bundle: Bundle(for: BundleClingClass.self),
      comment: "")
  }
...
extension NetworkHttpStatusCode {

  // MARK: - Error

  var error: Error? {
    if self.isSuccess == false {
      return NSError(domain: "NetworkHttpStatusCode", code: rawValue, userInfo: [
        NSLocalizedDescriptionKey: self.description,
        NSLocalizedFailureReasonErrorKey: self.localizedReasonPhrase
      ])
    }
...
public extension NetworkHttpStatusCode {
  ...
  init?(HTTPResponse: URLResponse?) {
    let response = HTTPResponse as? HTTPURLResponse

    self.init(HTTPResponse: response)
  }
  ...
  private init?(_ rawValue: Int) {
    guard let value = NetworkHttpStatusCode(rawValue: rawValue) else {
      return nil
    }
